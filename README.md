...
没有则自动创建程序集/子程序并改名定位。全程进程内 API 完成，不使用剪贴板、不修改 e5.95.exe、不做内存补丁。

- 当前版本：**1.6.9（INSERT_NEW_SUB_RENAME）**，全链路已在 e5.95 实测通过
...
| 代码写入 | 已有子程序 → `FN_MOVE_CARET` 跳转；没有 → 原生菜单插入程序集 + `FN_INSERT_NEW_SUB` 建子程序 + `FN_SET_AND_COMPILE_PRG_ITEM_TEXT` 改名（与 IDE 名称单元格编辑同一提交路径） |
...
产物：`bin\Release\JadeDesigner.fne`（约 330KB）与 `bin\Release\JadeDesigner\web\index.html`。

## 安装与测试

1. 完全关闭易语言。
2. 禁用/移走 `JadeDesignerProbe.fne`（避免日志混淆）；不要动 AutoLinker。
3. 复制 `JadeDesigner.fne` 与 `JadeDesigner\` 文件夹到 `%E_LANG_HOME%\lib\`。

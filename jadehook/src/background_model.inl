...
std::string HexName(const char* name) {
    if (!name) return "null";
    const size_t length = strnlen_s(name, 512);
    if (length >= 512) return "unterminated";
...
int EnsureBackground(const char* callbackAssemblyName, const char* assemblyName, const char* fixedName, const char* handlerName,
    ...
    if (!pending.empty()) {
        ModelItem parsed, firstParsed;
        const int parsedAssemblies = ModelCount(additions.bytes + 640);
        const int parsedRoutines = ModelCount(additions.bytes + 612);
        const int fixedMatches = !fixedCount
            ? FindModel(additions.bytes + 612, fixedName, parsed)
            : 1;
        const int handlerMatches = sameAssembly && !handlerCount
            ? FindModel(additions.bytes + 612, handlerName, parsed)
            : 1;
        const char* actualName = nullptr;
        if (parsedRoutines > 0) {
            firstParsed = ModelAt(additions.bytes + 612, 0);
            if (firstParsed.value) actualName = reinterpret_cast<const char*>(Ptr(firstParsed.value, 0));
        }
        if (parsedAssemblies != (assemblies ? 0 : 1) ||
            parsedRoutines != (!fixedCount + (sameAssembly && !handlerCount)) ||
            fixedMatches != 1 || handlerMatches != 1) {
            const std::string detail = "background_package_shape_invalid assemblies=" +
                std::to_string(parsedAssemblies) +
                " expected_assemblies=" + std::to_string(assemblies ? 0 : 1) +
                " routines=" + std::to_string(parsedRoutines) +
                " expected_routines=" + std::to_string(!fixedCount + (sameAssembly && !handlerCount)) +
                " fixed_matches=" + std::to_string(fixedMatches) +
                " handler_matches=" + std::to_string(handlerMatches) +
                " expected_handler_hex=" + HexName(handlerName) +
                " actual_first_hex=" + HexName(actualName);
            Reason(detail.c_str()); return -1;
        }
    }

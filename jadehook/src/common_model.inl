...
bool HasComparableLine(const std::string& source, const std::string& wanted) {
    const auto needle = Comparable(wanted);
    if (needle.empty()) return false;
    std::istringstream lines(source);
    std::string line;
    while (std::getline(lines, line))
        if (Comparable(line) == needle) return true;
    return false;
}

// The IDE serializer does not expose every assembly-level declaration as
// source text. Model and routine checks below are the authoritative validation.
bool AssemblyShapeMatches(const std::string& actual, const std::string& expected) {
    (void)expected;
    return !actual.empty() && !Comparable(actual).empty();
}
...
int EnsureCommon(const char* name, const char* source, const char* subscriptionName,
    ...
    bool ok = true;
    bool verifyFailureReported = false;
    const auto failStage = [&](const char* stage) {
        if (!ok && !verifyFailureReported) {
            Reason(stage);
            verifyFailureReported = true;
        }
    };
    if (!fixedCount) {
    ...
    failStage("common_verify_model_shape_failed");
    std::string actual;
    ok = ok && SerializeAssembly(project,common,actual) && AssemblyShapeMatches(actual, expectedAssembly);
    failStage("common_verify_assembly_structure_failed");
    for (const auto& apiName : apiNames)
        ok = ok && FindModel(project + 668,apiName.c_str(),parsed) == 1;
    failStage("common_verify_api_failed");
    for (const auto& subName : names)
        ok = ok && FindSub(project,common.value,subName.c_str(),parsed) == 1;
    failStage("common_verify_routine_failed");
    if (ok && fixedCount) {
        std::string after;
        ok = SerializeAssembly(project,subscription,after) && after == original;
        failStage("common_verify_subscription_changed");
    }
    ok = ok && *At<uintptr_t*>(0x675B44) == active;
    failStage("common_verify_active_editor_changed");
    undo.Close();
    if (!ok) {

...
bool SupportedHost() {
    ...
    static const bool matched = [] {
        ...
        if (!ok || size != sizeof(expected) || memcmp(digest, expected, size) != 0) {
            LARGE_INTEGER fileSize{};
            GetFileSizeEx(file, &fileSize);
            char ansiPath[MAX_PATH]{};
            WideCharToMultiByte(CP_ACP, 0, path, -1, ansiPath,
                static_cast<int>(sizeof(ansiPath)), nullptr, nullptr);
            char actualHash[65]{};
            for (DWORD i = 0; i < size && i < 32; ++i)
                sprintf_s(actualHash + i * 2, 65 - i * 2, "%02X", digest[i]);
            char detail[512]{};
            sprintf_s(detail, "memory_host_fingerprint_mismatch path=%s size=%lld sha256=%s",
                ansiPath, fileSize.QuadPart, actualHash);
            Reason(detail);
        }
...
extern "C" __declspec(dllexport) int WINAPI JadeHookListAssemblyNames(char* buffer, int capacity) {
    Operation op; if (!op.entered) return -1;
    BYTE* project = Project();
    const int count = ModelCount(project + 640);
    if (count < 0) { Reason("background_assembly_table_invalid"); return -1; }
    std::string names;
    for (int i = 0; i < count; ++i) {
        const ModelItem item = ModelAt(project + 640, i);
        if (!item.value || !Ptr(item.value, 0)) {
            Reason("background_assembly_name_invalid"); return -1;
        }
        if (!names.empty()) names += '\n';
        names += reinterpret_cast<const char*>(Ptr(item.value, 0));
        if (names.size() > kMaxSource) {
            Reason("background_assembly_names_too_large"); return -1;
        }
    }
    const int length = static_cast<int>(names.size());
    if (!buffer || capacity <= length) return length;
    memcpy(buffer, names.data(), names.size());
    buffer[names.size()] = '\0';
    Reason("background_assembly_names_read_success");
    return length;
}

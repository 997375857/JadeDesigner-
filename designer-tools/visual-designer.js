import {createElement,MousePointer2,Type,Heading2,TextCursorInput,ListFilter,CheckSquare,SquareDot,AlignLeft,Square,ChartNoAxesColumn,Save,Undo2,ArrowUp,ArrowDown,ArrowLeft,ArrowRight,RotateCcw,Copy,ScanSearch,Trash2} from 'lucide';
import {controls,containers,semanticControls,describeElement,resolveElement,propertyEdits,removeEdit,insertionEdit,encodeEdits,applySavedProperties} from './visual-model.js';
import {elementDescription} from './element-description.js';
import {metadataFor} from './jade-control-metadata.js';
...
export function createVisualDesigner({root,panel,body,foot,status,open,request,isTool,isDesign,notify}) {
  ...
  const style=el('style');style.textContent=`
    ...
    .visual-identity{font-size:12px;overflow-wrap:anywhere;margin:10px 0;color:#68736c}.visual-note{font-size:12px;color:#68736c;margin:8px 0}.visual-selection{position:fixed;border:2px solid #138757;pointer-events:none;box-sizing:border-box;border-radius:2px;z-index:2147483646;box-shadow:0 0 0 1px #fff8}
    .visual-panel .visual-save-note{font-size:12px;overflow-wrap:anywhere}
    .visual-module-capabilities{margin:8px 0;padding:9px;border:1px solid #d9e0dc;border-radius:4px;background:#f6faf7;color:#3d5548;font-size:12px;line-height:1.7;overflow-wrap:anywhere}.visual-module-capabilities strong{display:block;color:#195d39;margin-bottom:3px}
  ...
  const observer=new ResizeObserver(position);
   const typeOf=e=>metadataFor(e)?.label||({button:'按钮',label:'标签',span:'文字',p:'段落',input:'输入框',select:'下拉框',textarea:'多行输入',progress:'进度条',img:'图片',div:'布局区域',section:'区域',main:'区域',body:'页面',form:'表单'}[e.localName]||(/^h[1-6]$/.test(e.localName)?'标题':'区域'));
  const labelOf=e=>((e.matches('button,label,span,p,h1,h2,h3,h4,h5,h6')?(e.textContent||'').trim().slice(0,24):'')||e.getAttribute('data-jade-name')||e.getAttribute('data-jade-handler')||e.id||typeOf(e));
  ...
  function renderProperties() {
    ...
    const capabilities=metadataFor(selected);
    if(capabilities) {
      const box=el('div');box.className='visual-module-capabilities';
      box.append(el('strong','JadeView 模块能力：'+capabilities.label));
      if(capabilities.methods?.length)box.append(el('div','公开方法：'+capabilities.methods.map(([name])=>name).join('、')));
      if(capabilities.events?.length)box.append(el('div','可绑定事件：'+capabilities.events.map(({label})=>label).join('、')));
      body.append(box);
    }
    ...
    const remove=button('删除控件',async()=>{
      if(saving||!selected||!descriptor)return;
      if(!confirm('确定删除当前 HTML 控件吗？原有易语言回调和订阅不会自动删除。'))return;
      const version=selectionVersion,target=selected,snapshot=descriptor;saving=true;remove.disabled=true;
      try {
        current();const patch=removeEdit(source,descriptor);
        const [revision,next]=await request('visual_save',documentRevision,encodeEdits(patch));
        if(!target.isConnected||JSON.stringify(describeElement(target))!==JSON.stringify(snapshot))throw new Error('HTML 已保存，但页面发生变化，请重新加载网页核对');
        documentRevision=revision;source=next;selected=null;descriptor=null;render();status.textContent='控件已删除并备份；易语言回调和订阅请人工检查';
      }catch(error){if(stillSelected(version))fail(error);}finally{saving=false;remove.disabled=false;}
    },Trash2);
    foot.prepend(remove,save,undo);
  }
  ...
  function renderDescription() {
    ...
    const copyDescription=async(detailed,control)=>{
      control.disabled=true;
      try {
        const current=elementDescription(selected,{source,sourcePath,sourceError,detailed});
        text.value=current;
        await request('copy_description',current+'\n\n我的修改要求：\n'+(wanted.value.trim()||'（请补充修改要求）'));
        status.textContent=detailed?'详细元素描述已复制':'元素描述已复制';
      }catch(error){fail(error);}finally{control.disabled=false;}
    };
    const copy=button('复制给 AI',()=>copyDescription(false,copy),Copy);
    const copyDetailed=button('复制详细信息',()=>copyDescription(true,copyDetailed),ScanSearch);
    foot.prepend(copyDetailed,copy);
  }
  ...
  window.addEventListener('click',e=>{
    ...
    if(picking){disarm();visibleView='description';target=target?.closest(`${semanticControls},button,input,select,textarea,a,[role="button"]`)||target;if(target)select(target);return;}
    if(armedKind) {
    ...
    const semantic=target?.closest(semanticControls);
    if(semantic)target=semantic;
    else if(target?.closest('button'))target=target.closest('button');

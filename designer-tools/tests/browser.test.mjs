...
test('embedded bridge: manager, preview, edit isolation and responsive layout',async()=>{
  ...
  try {
    ...
    await tool(page,'通信诊断');
    await page.getByText('生成器已应答',{exact:true}).waitFor();
    assert.equal(await page.getByRole('button',{name:'接入运行诊断',exact:true}).count(),1);
...
test('runtime diagnostics observes real promises without payload logging',async()=>{
  ...
  try{
    ...
    await page.getByRole('button',{name:'运行通信诊断',exact:true}).click();
    ...
    const box=await page.getByRole('dialog',{name:'运行通信诊断',exact:true}).boundingBox();

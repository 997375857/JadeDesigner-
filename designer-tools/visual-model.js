...
export const controls='button,label,span,p,h1,h2,h3,h4,h5,h6,input,select,textarea,progress,img,div,section,main,article,aside,header,footer,nav,form,table,thead,tbody,tfoot,tr,th,td';
export const containers='div,section,main,article,aside,header,footer,nav,form,table,body';
// A data grid is one logical control even when a click lands on a cell,
// header, empty-state label, or a virtualized row.
export const semanticControls='[data-jade-control="list"],[data-jade-control="tree"],[role="grid"],[role="treegrid"],table';
...
export function removeEdit(source,descriptor) {
  const {node}=resolveElement(source,descriptor),location=node.sourceCodeLocation;
  if(!location?.startTag||!location.endTag||node.tagName==='body')throw new Error('该控件没有明确源码范围，不能删除');
  return [{kind:'remove',start:location.startTag.startOffset,end:location.endTag.endOffset,old:source.slice(location.startTag.startOffset,location.endTag.endOffset),key:'',value:''}];
}

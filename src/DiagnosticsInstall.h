...
inline constexpr wchar_t Tag[]=L"\n<script src=\"./jade-designer-diagnostics-v1.js?v=0912\" data-jade-diagnostics=\"1\"></script>\n";
...
inline bool Install(const std::wstring& html,const std::string& script,std::string& message) {
    ...
    const bool exactTag=source.compare(pos,std::wstring_view(Tag).size(),Tag)==0;
    const bool managedReference=source.find(FileName)!=source.npos&&
        source.find(L"data-jade-diagnostics=\"1\"")!=source.npos;
    if(!exactTag&&source.find(FileName)!=source.npos&&!managedReference){
        message="网页已有同名但非 Jade 诊断脚本引用，未修改";
        return false;
    }
    ...
    if(h!=INVALID_HANDLE_VALUE){
        ...
        if(!same){
            // This is our managed script from an earlier build. Upgrade only the
            // script file; never rewrite application HTML or add a second tag.
            h=CreateFileW(path.c_str(),GENERIC_WRITE,FILE_SHARE_READ,nullptr,CREATE_ALWAYS,0,nullptr);
            DWORD written=0;
            const bool ok=h!=INVALID_HANDLE_VALUE&&DesignerText::AtExpectedPath(h,path)&&
                WriteFile(h,script.data(),static_cast<DWORD>(script.size()),&written,nullptr)&&
                written==script.size()&&FlushFileBuffers(h);
            if(h!=INVALID_HANDLE_VALUE)CloseHandle(h);
            if(!ok){message="诊断脚本升级失败，HTML 未修改；请检查同目录诊断脚本";return false;}
        }
    }
    bool htmlChanged=false;
    if(managedReference&&!exactTag){
        const auto filePos=source.find(FileName);
        if(filePos!=source.npos){
            std::wstring versionedFile=FileName;
            versionedFile+=L"?v=0912";
            source.replace(filePos,std::wstring_view(FileName).size(),versionedFile);
            htmlChanged=true;
        }
    }
    if(!exactTag&&!managedReference){
        source.insert(pos,Tag);
        htmlChanged=true;
    }
    if(htmlChanged){
        if(!document.WriteChecked(DesignerText::Utf8(source),message))return false;
    }
    message=(exactTag||managedReference)?"运行诊断已升级，网页未重复插入引用。":"运行诊断已接入并备份 HTML；需在实际程序重新加载网页，不代表程序已经通过运行测试。";
    return true;
}
}
